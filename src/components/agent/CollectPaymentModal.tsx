...
interface CollectPaymentModalProps {
  ...
  prefillCustomer?: { name: string; email: string; phone: string } | null;
}

type PaymentMethod = "card" | "ach";
type PaymentMode = "charge" | "invoice";

export function CollectPaymentModal({ open, onOpenChange, prefillCustomer }: CollectPaymentModalProps) {
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (newOpen && prefillCustomer) {
      setForm(prev => ({
        ...prev,
        customerName: prefillCustomer.name || prev.customerName,
        customerEmail: prefillCustomer.email || prev.customerEmail,
      }));
    }
    if (!newOpen) {
      setMode(null);
  ...
  const handleSendInvoice = async () => {
    ...
    if (!form.customerEmail) {
      toast.error("Please enter customer email for invoice");
      return;
    }
        ...
        {isComplete ? (
          ...
          <div className="py-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                ${Number(form.amount).toLocaleString()} • {form.customerName}
              </p>
            </div>
            <Badge variant="outline" className="text-xs gap-1">
              <FileText className="w-3 h-3" />
              Receipt #INV-{Math.floor(Math.random() * 9999).toString().padStart(4, "0")}
            </Badge>
            <div className="pt-2">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                variant="outline"
                className="h-auto py-5 flex-col gap-2 hover:border-primary hover:bg-primary/5"
                onClick={() => setMode("charge")}
                disabled={!form.amount}
              >
                <CreditCard className="w-6 h-6" />
                <span className="text-sm font-semibold">Charge Now</span>
                <span className="text-[10px] text-muted-foreground">Process card/ACH payment</span>
              </Button>
              <Button
                variant="outline"
                className="h-auto py-5 flex-col gap-2 hover:border-primary hover:bg-primary/5"
              ...
              >
                ...
                <span className="text-[10px] text-muted-foreground">Email invoice to pay later</span>
              </Button>
            </div>
          </div>
        ) : mode === "charge" ? (
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
              ...
              <span>Invoice will be emailed with a payment link (demo mode)</span>
            </div>
