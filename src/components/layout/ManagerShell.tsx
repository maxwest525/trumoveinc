    ...
    <div className="flex min-h-screen bg-background text-foreground">
      {!isMobile && (
        <aside className={cn("shrink-0 border-r border-border flex flex-col min-h-screen transition-all duration-200", sidebarWidth)}>
          <SharedSidebar title="Manager" collapsed={collapsed} onToggleCollapse={() => setCollapsed((c) => !c)} isMobile={false} />
        </aside>
      )}
      ...
      {isMobile && sidebarOpen && (
        <>
          ...
          <aside className="fixed inset-y-0 left-0 w-56 z-50 border-r border-border flex flex-col">
            <SharedSidebar title="Manager" collapsed={false} onToggleCollapse={() => {}} onClose={() => setSidebarOpen(false)} isMobile={true} />
          </aside>
        </>
      )}
